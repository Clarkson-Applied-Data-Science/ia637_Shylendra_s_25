...
class user(baseObject):
    def __init__(self):
        self.setup()
        self.fetch_roles_from_db()
        # self.roles = [
        #     {'value': 'admin', 'text': 'admin'},
        #     {'value': 'seller', 'text': 'seller'},
        #     {'value': 'buyer', 'text': 'buyer'},
        # ]
    def fetch_roles_from_db(self):
        sql = "SELECT DISTINCT role FROM Users"
        self.cur.execute(sql)
        roles = self.cur.fetchall()
        self.roles = [{'value': row['role'], 'text': row['role']} for row in roles if row['role'] != 'customer']
    ...
    def get_all_admins(self):
        sql = f"SELECT * from {self.tn} Where role = 'admin'"
        self.cur.execute(sql)
        return self.cur.fetchall()
    def get_all_sellers(self):
        sql = f"SELECT * from {self.tn} Where role = 'seller'"
        self.cur.execute(sql)
        return self.cur.fetchall()
    def get_all_buyers(self):
        sql = f"SELECT * from {self.tn} Where role = 'buyer'"
        self.cur.execute(sql)
        return self.cur.fetchall()